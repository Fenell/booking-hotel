...
const Switch = ({ checked, id = "toggle", onToggle }: SwitchProps) => {
  return (
    <label className={switchStyle["switch"]}>
      ...
      <div className={switchStyle["slider"]}>
        <div className={switchStyle["circle"]}>
          ...
          {/* ✅ icon */}
          <svg
            className={switchStyle["checkmark"]}
            viewBox="0 0 24 24"
            height="10"
            width="10"
            xmlns="http://www.w3.org/2000/svg"
    ...
    // <div className={switchStyle["toggle-switch"]}>
    //   <input
    //     className={switchStyle["toggle-input"]}
    //     id={id}
    //     type="checkbox"
    //     checked={value}
    //     onChange={handleChange}
    //   />
    //   <label className={switchStyle["toggle-label"]} htmlFor={id}></label>
    // </div>
  );
};
